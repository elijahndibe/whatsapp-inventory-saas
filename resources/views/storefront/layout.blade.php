    ...
    <body class="font-sans antialiased bg-surface dark:bg-gray-900">
        ...
        <header class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
            <div class="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
                ...
                <a href="{{ route('storefront.cart.index', $business) }}"
                   class="relative inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition shrink-0"
                   aria-label="{{ $cartCount > 0 ? trans_choice(':count item in cart|:count items in cart', $cartCount, ['count' => $cartCount]) : __('Cart') }}">
                    ...
                    <span class="hidden sm:inline" aria-hidden="true">{{ __('Cart') }}</span>
                    @if ($cartCount > 0)
                        <span class="absolute -top-1.5 -right-1.5 inline-flex items-center justify-center h-5 min-w-5 px-1 rounded-full bg-brand-700 text-white text-[11px] font-semibold" aria-hidden="true">{{ $cartCount }}</span>
                    @endif
                </a>
            </div>
        </header>
        ...
        <main id="main-content" tabindex="-1" class="max-w-5xl mx-auto px-4 py-6">
            {{ $slot }}
        </main>
